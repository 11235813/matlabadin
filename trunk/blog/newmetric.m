...
for i=1:size(tmi_data,1)
    if tmi_data(i,:)<0
        tmi_data(i,:)=-tmi_data(i,:);        
    end
    if tmiw_data(i)<0
        tmiw_data(i)=-tmiw_data(i);
    end
end
...
li{1+(1:length(pct_param_list)),2:size(tmi_data,1)}=round(tmi_data(2:size(tmi_data,1),:))';
...
li{1,2:size(tmi_data,1)}=cellstr({statSetup(2:length(statSetup)).name});
li{1+(1:length(hdf_list)),2:size(tmi_data,1)}=round(tmi_data(2:size(tmi_data,1),:))';
...
li{1+(1:length(hdf_list)),1}=hdf_list';
li{1,2:size(tmi_data,1)}=cellstr({statSetup(2:length(statSetup)).name});
li{1+(1:length(hdf_list)),2:size(tmi_data,1)}=round(tmiw_data(2:size(tmi_data,1),:))';
li.setColumnFormat(1,'%1.1f')
disp('<pre>')
