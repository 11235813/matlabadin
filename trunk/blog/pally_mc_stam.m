...
hasteStep=1e3;
stamStep=hasteStep*3/4;
while statSetup(i).hasteRating>0
    i=i+1;
    statSetup(i)=statSetup(i-1);
    statSetup(i).hasteRating=statSetup(i-1).hasteRating-hasteStep;
    statSetup(i).stamina=statSetup(i-1).stamina+stamStep;    
    statSetup(i).name=['C' int2str(i)];
    statSetup(i).name=['H' int2str(round(statSetup(i).hasteRating./1e3)) 'S' int2str(i)];
end
...
%% save data
fbase=['.\pdata\pally_stam_data_' int2str(config.simMins)];
i=0;
while exist([fbase '_' int2str(i) '.mat'])==2
    i=i+1;
end
config.fileid=i;
config.filetype='stam';
