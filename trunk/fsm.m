...
function  [action, pr] = fsm(graphParameters, printTransitions = 0)
global fsmInternal;
% unit testing hack
if nargin == 0
...
function fsm_init (stepSize = 0.5, usingConsGlyph = 0)
	global fsmInternal;
	...
	fsmInternal.StateEncodingProducts = cumprod(uint64([ 1 fsmInternal.StateSpaceSize(1:end-1)]));
...
function [M, stateVector] = fsm_generateStateTransitionMatrix(graphParameters, initialState, printTransitions = 0)
	global fsmInternal;
	...
	stateLookup = struct([]); % using struct as a hashmap. Run perftest.m to ensure ensure this is reasonable for matlab as well as Octave
	...
	while stateIndex <= totalStateSpaceSize
		...
		if mod(stateIndex, 1000) == 0
			warning(sprintf("Generated %d states so far", stateIndex));
		end
	end
	...
	if totalStateSpaceSize > 1000
		warning(sprintf("State size is %d states", totalStateSpaceSize));
	end
end
...
function nextState = fsm_calculateNextState(state, waitSteps, abilityName, miss = 0, egProc = 0, sdProc = 0, gcProc = 0)
	global fsmInternal
	hopo = state(fsmInternal.HoPoOffset);
