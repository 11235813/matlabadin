...
raw.SealofRighteousness=    gear.swing.*(0.011.*player.ap+0.022.*player.hsp).* ...
                            mdf.spdmg.*mdf.SotP;
raw.SealJud(2)=             (1+0.25.*player.hsp+0.16.*player.ap);
...
%Crusader Strike
raw.CrusaderStrike= player.wdamage.*mdf.phdmg.*mdf.Crus.*(1+2.*mdf.WotL);
...
raw.HammerNova=   ((523+783)/2).*mdf.spdmg.*mdf.Crus;  %Base damage is lvl 80, TODO: fix
...
raw.ShieldoftheRighteous= (mdf.ShoR./100.*player.ap).*mdf.spdmg; 
...
%Avenger's Shield
raw.AvengersShield= ((2512+3070)/2).*mdf.spdmg;
...
%Hammer of Wrath
raw.HammerofWrath=  ((1124+1242)/2 + 0.15.*player.hsp + 0.15.*player.ap).*mdf.spdmg;
dmg.HammerofWrath= raw.HammerofWrath.*mdf.rahit.*mdf.HoWcrit.*target.resrdx;

%% Spell abilities

%Consecration
raw.Consecration =  (8.*(0+0.027.*player.hsp+0.027.*player.ap)).*mdf.spdmg.*mdf.HalGro;
...
raw.Exorcism=       (1018+1136)/2 + 0.15.*max([player.hsp; player.ap]).*mdf.spdmg;
...
%Holy Wrath
raw.HolyWrath=      (2153 + 0.3.*player.hsp).*mdf.spdmg;
