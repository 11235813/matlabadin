...
while i>0 && dt>=0 && dt<=10
    ...
    if contribution>0 %&& (any sort of cap calculation)
        DamageMod=DamageMod+contribution;
    end
...
%divide by "basic equal-level creature damage", hardcoded for level 100 player
DamageMod = DamageMod/(10*3615.3);
newValue=max([0 8.5*(1-exp(-0.045*DamageMod))-1 ]);
