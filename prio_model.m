...
cd.CS=3;cd.Jud=8;cd.AS=15;cd.HoWr=15;cd.Cons=30.*mdf.glyphCons;
...
priodefault.labels={'3ShoR';'CS';'Jud';'AS';'HW';'Cons';'HotR';'HaNova';'2ShoR';'Inq'};
priodefault.cds=[0 cd.CS cd.Jud cd.AS cd.HoWr cd.Cons cd.CS 0 0 0];
%define gcd lengths
priodefault.gcds=[1.5 1.5 1.5 1.5 player.spgcd player.spgcd 1.5 1.5 1.5 player.spgcd];
priodefault.gcds=[1.5 1.5 1.5 1.5 1.5 1.5 1.5 0 1.5 1.5];
priodefault.ids=[1 2 3 4 5 6 2 0 0 0];  %id numbers for generating figures - 0 is not plotted
...
priodefault.action={'hopo=0;dur.SD=0;';...          %todo: Does a missed ShoR reset SD?
                ...
                ' '; ...                        %HammerNova never gets cast without HotR
...
k=k+1;
prio(k)=prio(1);
prio(k).name='SD>ShoR>CS>AS>J>HW (no Cons)';
prio(k).cond={'hopo>=3 && ccd.Jud<=0 && dur.SD<=0';...
              'hopo>=3';...
              'ccd.CS<=0';...
              'ccd.AS<=0';...
...
k=k+1;
prio(k)=prio(1);
prio(k).name='ShoR>CS>AS>J>HW (no SD)';
prio(k).cond={'hopo>=3';...
              'ccd.CS<=0';...
              'ccd.AS<=0';...
              'ccd.Jud<=0';...
              'ccd.HoWr<=0'};
prio(k).cast=[1;2;4;3;5];

k=k+1;
prio(k)=prio(1);
prio(k).name='ShoR>AS>CS>J>HW (no SD)';
prio(k).cond={'hopo>=3';...
              'ccd.AS<=0';...
              'ccd.CS<=0';...
              'ccd.Jud<=0';...
              'ccd.HoWr<=0'};
prio(k).cast=[1;4;2;3;5];

k=k+1;
prio(k)=prio(1);
prio(k).name='AS>ShoR>CS>J>HW (no SD)';
prio(k).cond={'ccd.AS<=0';...
              'hopo>=3';...
              'ccd.CS<=0';...
              'ccd.Jud<=0';...
              'ccd.HoWr<=0'};
prio(k).cast=[4;1;2;3;5];
...
prio(k).cond={'hopo>=3 && ccd.Jud<=0 && dur.SD<=0';...
              ...
              'ccd.AS<=0';...
              'ccd.HoWr<=0'};
prio(k).cast=[3;1;9;2;3;4;5];

k=k+1;
prio(k)=prio(1);
prio(k).name='Inq>3ShoR>CS>J>AS>HW (Inq model)';
...
k=k+1;
prio(k)=prio(1);
prio(k).name='Inq>3ShoR>2SDShoR>CS>J>AS>HW (Inq model)';
prio(k).cond={'hopo>=3 && dur.Inq<=0';...
              'hopo>=3';...
              'hopo>=2 && dur.SD>0';...
              'ccd.CS<=0';...
              ...
              'ccd.AS<=0';...
              'ccd.HoWr<=0'};
prio(k).cast=[10;1;9;2;3;4;5];

k=k+1;
prio(k)=prio(1);
prio(k).name='ShoR>HotR>J>AS>HW (no SD)';
prio(k).cond={'hopo>=3';...
              'ccd.CS<=0';...
              'ccd.Jud<=0';...
              'ccd.AS<=0';...
